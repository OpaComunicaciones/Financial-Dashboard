...
const BudgetVsActualReport: React.FC<ReportProps> = ({ startDate, endDate, reportingCurrency }) => {
  ...
  const comparisonData = useMemo(() => {
    ...
    state.cashExpenses.forEach(expense => {
      // Skip if it's a payment of an invoice track above
    ...
    state.transactions.forEach(tx => {
      // Skip if it's a payment of an invoice
      if (tx.isPayment || tx.description.includes('Payment for invoice #')) return;
      if (tx.type === 'expense' && tx.conceptId) {
  ...
  const handleExportPDF = () => {
    ...
    const incomeBody: any[] = comparisonData.incomeRows.map(row => {
    ...
    const expenseBody: any[] = comparisonData.expenseRows.map(row => {
      const variancePercent = row.budgeted !== 0 ? formatNumber((row.variance / row.budgeted) * 100) : 'N/A';
      return [row.name, formatCurrency(row.budgeted), formatCurrency(row.actual), formatCurrency(row.variance), `${variancePercent}%`];
    });
    expenseBody.push([
  ...
  const renderRow = (row: { name: string, budgeted: number, actual: number, variance: number }) => {
    ...
    const varianceColor = isFavorable ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
    const variancePercent = row.budgeted !== 0 ? formatNumber((row.variance / row.budgeted) * 100) : 'N/A';

    return (
      <tr key={row.name} className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
        <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{row.name}</td>
        <td className="px-6 py-4 text-right font-mono text-gray-700 dark:text-gray-300">{formatCurrency(row.budgeted)}</td>
        <td className="px-6 py-4 text-right font-mono text-gray-700 dark:text-gray-300">{formatCurrency(row.actual)}</td>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <h3 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400 p-6">{t('reports_tab_budget')} ({t('reports_consolidated_in')} {reportingCurrency})</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <thead className="text-xs text-gray-500 dark:text-gray-400 uppercase bg-gray-50 dark:bg-gray-700/50 backdrop-blur-sm">
              <tr><th colSpan={5} className="px-6 py-4 text-green-600 dark:text-green-400 font-bold text-lg border-b border-gray-200 dark:border-gray-600">{t('reports_income_header')}</th></tr>
              <tr>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
              ...
              <tr className="bg-gray-50 dark:bg-gray-900/50 font-bold border-t-2 border-gray-200 dark:border-gray-600">
                <td className="px-6 py-4 text-right text-gray-900 dark:text-white">{t('reports_total_income')}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{formatCurrency(comparisonData.incomeTotals.budgeted)}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{formatCurrency(comparisonData.incomeTotals.actual)}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{formatCurrency(comparisonData.incomeTotals.variance)}</td>
            ...
            {/* Expense Section */}
            <thead className="text-xs text-gray-500 dark:text-gray-400 uppercase bg-gray-50 dark:bg-gray-700/50 border-t-2 border-gray-200 dark:border-gray-600">
              <tr><th colSpan={5} className="px-6 py-4 text-red-600 dark:text-red-400 font-bold text-lg border-b border-gray-200 dark:border-gray-600">{t('reports_expenses_header')}</th></tr>
              <tr>
                <th scope="col" className="px-6 py-3">{t('planning_category')}</th>
                <th scope="col" className="px-6 py-3 text-right">{t('reports_budget_budgeted')}</th>
                <th scope="col" className="px-6 py-3 text-right">{t('reports_budget_actual')}</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
              ...
              <tr className="bg-gray-50 dark:bg-gray-900/50 font-bold border-t-2 border-gray-200 dark:border-gray-600">
                <td className="px-6 py-4 text-right text-gray-900 dark:text-white">{t('reports_total_expenses')}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{formatCurrency(comparisonData.expenseTotals.budgeted)}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{formatCurrency(comparisonData.expenseTotals.actual)}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{formatCurrency(comparisonData.expenseTotals.variance)}</td>
                <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">
